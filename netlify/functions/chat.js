...
exports.handler = async (event) => {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      let userErrorMessage = 'O serviço de chat está indisponível neste momento. Tenta novamente em alguns segundos.';
      ...
      try {
        ...
        if (code === 'insufficient_quota') {
          userErrorMessage = 'A quota da OpenAI foi excedida ou os créditos terminaram. Verifica a tua conta OpenAI.';
        } else if (code === 'invalid_api_key') {
          userErrorMessage = 'A API key da OpenAI é inválida. Verifica as variáveis de ambiente na Netlify.';
        }
      } catch (e) {
        // Se não for JSON, mantemos a mensagem genérica
      }
